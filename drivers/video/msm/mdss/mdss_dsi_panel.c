...
{		
	...
	if (!strcmp(panel_name, "zfgDIJING(ili9881c+boe)_720*1280-5.0Inch"))
	{
		BLMAX_LCM=0x1;
	}
...
static void mdss_dsi_panel_bklt_dcs(struct mdss_dsi_ctrl_pdata *ctrl, int level)
{
	...
	//add end
		...
		pr_err("%s: level=%d,0x%x,0x%x\n", __func__, level,led_dcs1[1],led_dcs1[2]);
	}
	else
	{
		//add by yujianhua for minimum brightness
							...
							u32 bl_level)
{
	...
	case BL_DCS_CMD:
		...
		if(bl_level != 0){
		    int retval1, retval2;
	        retval1 = gpio_direction_output(tp_gpio_reset, 0);
	        msleep(50);
			retval2 = gpio_direction_output(tp_gpio_reset, 1);
			printk("wkh mdss_dsi_panel_bl_ctrl, retval1=%d retval2=%d\n", retval1,retval2);
		}
                //end*/
