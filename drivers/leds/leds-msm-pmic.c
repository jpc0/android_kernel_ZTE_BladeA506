...
// 修改关机充电指示灯不亮的问题chenyingfang 20141014 ,start
...
{
// 修改关机充电指示灯不亮的问题,增加charge_mode判断. chenyingfang 20141014 ,start
#ifdef CONFIG_MODEM_LED
...
//should always define CONFIG_MODEM_LED, this code only for test current,never used for normal purpose
    ...
    // 由于AP侧休眠可能导致定时器中断，出现红、绿灯无法关闭的情况，在suspend先直接拉GPIO的方式处理
	int i;
	for (i = 0; i < 2; i++)
