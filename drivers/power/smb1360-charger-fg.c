...
#ifdef CONFIG_ZFG_BATTERY_CHG_DEGREE_0_45
...
static int smb1360_select_fg_i2c_address(struct smb1360_chip *chip)
{
	...
	pr_err("FG_access_type=%d fg_i2c_addr=%x\n", chip->fg_access_type,
							chip->fg_i2c_addr);
...
static int smb1360_enable_fg_access(struct smb1360_chip *chip)
{
	...
	if (rc)
		...
		if (!timeout) {
			pr_debug("reset fg to get the access again\n");
			rc = smb1360_masked_write(chip, CMD_I2C_REG,
	...
	if (rc) {
			...
			/* wait 2 seconds to wait for FG reset complete */
			msleep(2000);
	}
		retry--;
	}
...
void smb1360_otp_backup_pool_init(struct smb1360_chip *chip)
{
	struct otp_backup_pool *pool = &chip->otp_backup;
        ...
        pr_err("smb1360_otp_backup_pool_init\n");
	pool->reg_start = 0xE0;
	pool->reg_end = 0xEF;
						...
						u8 size, int usage)
{
	...
	pr_err("Allocation success, start = 0x%x, size = %d, alg_bitmap = 0x%x\n",
...
static int smb1360_otp_backup_alg_update(struct smb1360_chip *chip)
{
	...
	mutex_unlock(&pool->lock);
        pr_err("smb1360_otp_backup_alg_update\n");
...
static int smb1360_set_appropriate_usb_current(struct smb1360_chip *chip)
{
	...
	pr_err("therm_lvl_sel=%d, thermal_levels=%d,path_current=%d,therm_ma=%d\n",chip->therm_lvl_sel,chip->thermal_levels,path_current,therm_ma);
	current_ma = min(therm_ma, path_current);

//only for battery with temperature windows of degree 0-45
#ifndef CONFIG_ZFG_BATTERY_CHG_DEGREE_0_45
	...
	//}
#endif
	pr_err("current_ma=%d,batt_warm=%d,batt_cool=%d\n",current_ma,chip->batt_warm,chip->batt_cool);
	if (current_ma <= 2) {
		/*
	...
	if ((current_ma <= CURRENT_100_MA) &&
		((chip->workaround_flags & WRKRND_USB100_FAIL) ||
				!chip->min_icl_usb100)) {
		pr_err("usb100 not supported: usb100_wrkrnd=%d min_icl_100=%d\n",
			!!(chip->workaround_flags & WRKRND_USB100_FAIL),
						chip->min_icl_usb100);
	...
	if (current_ma <= CURRENT_100_MA) {
		...
		pr_err("fast-chg current set to = %d, current_ma=%d\n", fastchg_current[i],current_ma);
#else
				...
				int cold_threshold, int hot_threshold)
{
	...
	pr_err("cold:%d, cool:%d, warm:%d, hot:%d\n",
			cold_threshold, chip->cool_bat_decidegc,
			chip->warm_bat_decidegc, hot_threshold);
	...
	pr_err("hard_jeita_otp_reg = 0x%x\n", chip->hard_jeita_otp_reg);
	...
	for (i = 0; i < ARRAY_SIZE(reg); i++) {
		...
		pr_err("Write FG addr=0x%x, value=0x%x\n",
					otp_reg + i, reg[i]);
	}
...
{
	int rc = 0;
        pr_err("smb1360_hard_jeita_otp_init\n");
	...
	if (rc) {
		dev_err(chip->dev,			"Couldn't set OTP hard jeita threshold,rc = %d\n", rc);
		return rc;
	}
							...
							int lvl_sel)
{
	...
	pr_err("set USB current to minimum therm_lvl_sel = %d, thermal_levels=%d\n",chip->therm_lvl_sel,chip->thermal_levels);
...
static void smb1360_external_power_changed(struct power_supply *psy)
{
	...
	pr_err("set usb current chip->usb_psy_ma = %d, current_limit=%d\n", chip->usb_psy_ma, current_limit);
...
static void smb1360_jeita_work_fn(struct work_struct *work)
{
	...
	if (temp > chip->hot_bat_decidegc) {
		...
		pr_err("warm will set USB current\n");
		rc = smb1360_set_appropriate_usb_current(chip);
		if (rc)
		...
		pr_err("cool will set USB current\n");
		rc = smb1360_set_appropriate_usb_current(chip);
		if (rc)
			pr_err("Couldn't set USB current\n");
		rc = smb1360_set_soft_jeita_threshold(chip,
		...
		pr_err("cold will set USB current\n");
		rc = smb1360_set_appropriate_usb_current(chip);
		if (rc)
			pr_err("Couldn't set USB current\n");
		rc = smb1360_set_soft_jeita_threshold(chip,
...
//only for battery with temperature windows of degree 0-45
#ifndef CONFIG_ZFG_BATTERY_CHG_DEGREE_0_45 
       pr_err("hot soft will set USB current\n");
...
//only for battery with temperature windows of degree 0-45
#ifndef CONFIG_ZFG_BATTERY_CHG_DEGREE_0_45 
	pr_err("cold soft will set USB current\n");
	smb1360_set_appropriate_usb_current(chip);
	smb1360_set_recharge_thresh_fg_source(chip);
#endif
						...
						int gain_factor)
{
	...
	pr_err("current_gain_otp_reg = 0x%x\n", chip->current_gain_otp_reg);
	...
	if (gain_factor) {
		...
		pr_err("current_gain_reg=0x%x current_gain_decoded=%lld new_current_gain_decoded=%lld new_current_gain_reg=0x%x\n",
			reg_value1, current_gain, new_current_gain, reg_value2);


		for (i = 0; i < ARRAY_SIZE(reg); i++) {
			pr_err("Writing reg_add=%x value=%x\n",
					otp_reg + i, reg[i]);
...
static int determine_initial_status(struct smb1360_chip *chip)
{
	...
	if (chip->workaround_flags & WRKRND_HARD_JEITA) {
		...
		pr_err("no WRKRND_HARD_JEITA\n");
		if (reg & IRQ_A_HOT_HARD_BIT)
			chip->batt_hot = true;
		if (reg & IRQ_A_COLD_HARD_BIT)
			chip->batt_cold = true;
		if (!chip->config_hard_thresholds) {
			pr_err("no config_hard_thresholds\n");
	...
	else if(chip->batt_cool == true)
	{
		pr_err("cool when init\n");
		temp = (chip->vfloat_mv - chip->cool_bat_mv) / 10;
		rc = smb1360_masked_write(chip, CFG_FVC_REG,
				FLT_VTG_COMP_MASK, temp);
		if (rc < 0) {
	...
	{
		//will finish config in smb1360_jeita_init
	}
...
static int smb1360_parse_jeita_params(struct smb1360_chip *chip)
{
	...
	if (of_property_read_bool(node, "qcom,config-hard-thresholds")) {
		...
		pr_err("otp_hard_jeita_config = %d, otp_cold_bat_decidegc = %d, otp_hot_bat_decidegc = %d\n",
