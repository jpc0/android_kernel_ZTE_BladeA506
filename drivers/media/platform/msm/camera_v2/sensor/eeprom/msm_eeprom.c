...
#define OV5648_SENSOR_INFO_MODULE_ID_GOERTEK		0x09
#define OV5648_SENSOR_INFO_MODULE_ID_SEASON		0x10
...
MODULE_Map_Table OV5648_MODULE_MAP[] = {
    { OV5648_SENSOR_INFO_MODULE_ID_GOERTEK,"goertek_ov5648", "goertek_ov5648",NULL},
    { OV5648_SENSOR_INFO_MODULE_ID_SEASON, "season_ov5648","season_ov5648",NULL},
};
	...
	if(index != -1){
		...
		*/
		if(map && (map[index].sensor_module_name)){
			...
			string_length = 0;
			pr_err("ZFG_CAMERA:%s:%d: sensor_module_name = %s\n",
		...
		*/
		if(map && (map[index].chromtix_lib_name)){
			...
			string_length = 0;
			pr_err("ZFG_CAMERA:%s:%d: chromtix_lib_name = %s\n",
		...
		*/
		if(map && (map[index].default_chromtix_lib_name)){		
			...
			pr_err("ZFG_CAMERA:%s:%d: default_chromtix_lib_name = %s\n",
...
if(strcmp(eb_info->eeprom_name,"ov8856_otp")==0){
	        sensor_module_id = e_ctrl->cal_data.mapdata[1];
                if(sensor_module_id==0x0||sensor_module_id==0xFF)
                sensor_module_id = e_ctrl->cal_data.mapdata[9];
                if(sensor_module_id==0x0||sensor_module_id==0xFF)
...
if(strcmp(eb_info->eeprom_name,"ov5648_otp")==0){
	        sensor_module_id = e_ctrl->cal_data.mapdata[5];
                if(sensor_module_id==0x0||sensor_module_id==0xFF)
                sensor_module_id = e_ctrl->cal_data.mapdata[14];
                if(sensor_module_id==0x0||sensor_module_id==0xFF)
                sensor_module_id = 0x09;
                msm_sensorinfo_set_back_sensor_module_id(sensor_module_id);     
		parse_module_name(e_ctrl,OV5648_MODULE_MAP,
			sizeof(OV5648_MODULE_MAP)/sizeof(MODULE_Map_Table),sensor_module_id);
}
	for (j = 0; j < e_ctrl->cal_data.num_data; j++)
		CDBG("memory_data[%d] = 0x%X\n", j,
			e_ctrl->cal_data.mapdata[j]);
