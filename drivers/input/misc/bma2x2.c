		...
		struct cal_result_t *cal_result)
{
	...
	data->cal_x = cal_result->offset_x;
	data->cal_y = cal_result->offset_y;	
	data->cal_z = cal_result->offset_z;	
	if (bma2x2_set_offset_x(data->bma2x2_client, (unsigned char)data->cal_x) < 0)
	{
		return -EINVAL;
	}
	
	if (bma2x2_set_offset_y(data->bma2x2_client, (unsigned char)data->cal_y) < 0)
	{
		return -EINVAL;
	}	
	
	if (bma2x2_set_offset_z(data->bma2x2_client, (unsigned char)data->cal_z) < 0)
	{
		return -EINVAL;
	}	
