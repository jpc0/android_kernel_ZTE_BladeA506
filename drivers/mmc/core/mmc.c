...
int i;
    ...
    {
	for (i = 0; i < ARRAY_SIZE(man_list); i++) {
		if (man_list[i].id == host->card->cid.manfid) {
			strcat(emmc_module_name , man_list[i].name);
			break;
		}
}
	//strcpy(emmc_module_name,"HYNIX 8GB eMMC");
if( host->card->ext_csd.sectors <8000000) 
			strcat(emmc_module_name ," 4G  EMMC");
else if( host->card->ext_csd.sectors <16000000) 
			strcat(emmc_module_name ," 8G  EMMC");
else if( host->card->ext_csd.sectors <32000000) 
			strcat(emmc_module_name ," 16G  EMMC");
else
			strcat(emmc_module_name ," 32G  EMMC");
}
