		      ...
		      const char *text, u16 text_len)
{
	...
	{
		zfg_ts = current_kernel_time();
		msg->ts_nsec = zfg_ts.tv_sec;
		msg->zfg_ts_nsec = zfg_ts.tv_nsec;
	}
...
#else
static size_t print_time(u64 ts, long zfg_ns, char *buf, 
...
#endif
{
	...
	tlen = sprintf(buf , "[%02d-%02d %02d:%02d:%02d.%03d] ",
		...
		tm.tm_sec, (int)(zfg_ns / NSEC_PER_MSEC));
...
#else
	len += print_time(msg->ts_nsec, msg->zfg_ts_nsec, buf ? buf + len : NULL, msg->process_id, msg->pid, msg->comm);
#endif
	return len;
}
...
#if 1
		zfg_ts = current_kernel_time();
		cont.ts_nsec = zfg_ts.tv_sec;
		cont.zfg_ts_nsec = zfg_ts.tv_nsec;
#endif
...
#else
		textlen += print_time(cont.ts_nsec, cont.zfg_ts_nsec, text,  cont.process_id, cont.pid, cont.comm);
#endif
		size -= textlen;
	}
...
static int __init printk_late_init(void)
{
	...
	printk("zfg log address __log_buf: 0x%p\n", __log_buf);	
	return 0;
}
